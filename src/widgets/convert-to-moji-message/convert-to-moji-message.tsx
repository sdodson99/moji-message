...
export function ConvertToMojiMessage() {
  ...
  const [lastCreateMojiMessageRequest, setLastCreateMojiMessageRequest] = useState<CreateMojiMessageRequest>();

  const onMojiMessageCreate = (mojiMessage: string, createMojiMessageRequest: CreateMojiMessageRequest) => {
      ...
      {mojiMessage ? (
        <section className="relative mt-12">
          <div className="absolute top-5 right-5">
            <CopyMojiMessageButton mojiMessage={mojiMessage} lastCreateMojiMessageRequest={lastCreateMojiMessageRequest} />
          </div>
